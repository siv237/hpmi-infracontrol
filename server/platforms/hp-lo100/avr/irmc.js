...
export class IrmcClient {
  constructor(opts, events = {}) {
    ...
    this.userStatusMask = undefined;   // 0 = полный контроль, иначе view-only/ephemeral
  ...
  wire(sig, user, pass, pwdFull, key) {
    ...
    // Паддинги полей зависят от BMC: iRMC — 48/48/228 (по умолчанию),
    // HP LO100 — 16/20/128 (см. CConn.init в M2.JAR; wiki hp-lo100-kvm.md).
    const P = this.opts.pad || {};
    const username = pad(user, P.user || 48);
    const password = pad(pass, P.pass || 48);
    const passwordFull = pad(pwdFull, P.full || 228);
  ...
  sendStartup() {
    ...
    // 211 RequestPrimaryControl — БЕЗ payload (Command.writeBuffer пишет
    // только байт типа; лишний байт BMC трактует как следующую команду и
    // десинхронизирует поток).
    this.sendCmd(ID.RequestPrimaryControl);                                // 0xd3
    this.invalidateFull();
  }
  ...
  onCommand(cmd) {
    ...
    switch (cmd) {
      ...
      case ID.MultiUserState: {
        ...
        const flags = r.u32();
        // getUserStatusMask() апплета: маска 24 (биты 3-4), сдвиг 3 ->
        // status = (flags & 0x18) >>> 3. 0 = полный контроль, 1 = view-only,
        // 2 = ephemeral, 3 = принудительный разрыв. В view-only BMC МОЛЧА
        // игнорирует KeyStateChange — симптом «клавиши не набираются»
        // (HP LO100 стартует сессию view-only). Паритет апплета
        // (CConn.onMultiUserState): status != 0 -> просим полный контроль
        // (211); получили 0 -> RequestKeyIndicators (193).
        const status = (flags & 0x18) >>> 3;
        const prev = this.userStatusMask;
        this.userStatusMask = status;
        const s0 = r.u8(), s1 = r.u8();
        const f0 = r.u8(), f1 = r.u8();
        const n0 = r.string(s0), n1 = r.string(s1);
        if (irmcDbg()) console.log(`[irmc][multiuser] flags=0x${(flags >>> 0).toString(16)} status=${status} users=[${JSON.stringify(n0)},${JSON.stringify(n1)}]`);
        if (status !== 0) {
          // Повторяем запрос на каждое новое состояние, пока не дадут
          // контроль; кап 3 попытки — если контроль держит другой пользователь.
          this._ctrlReqN = (this._ctrlReqN || 0) + 1;
          if (this._ctrlReqN <= 3) this.sendCmd(ID.RequestPrimaryControl);
        } else if (prev !== 0) {
          this._ctrlReqN = 0;
          this.sendCmd(ID.RequestKeyIndicators);
        }
        this.events.onStatus?.(`multiuser:${status}`);
        this.afterHandshake();
        break;
      }
  ...
  // ---- input ----------------------------------------------------------------
  key(scanCode, down) {
    // В view-only BMC дропает KeyStateChange без ответа — не шлём вовсе
    // (паритет апплета: writeKeyEvent гейтится полным контролем).
    if (this.userStatusMask) {
      if (irmcDbg()) console.log(`[irmc][kbd] drop (view-only, status=${this.userStatusMask})`);
      return;
    }
    this.sendCmd(ID.KeyStateChange); const b = Buffer.alloc(4); b.writeUInt16LE(scanCode, 0); b[2] = down ? 1 : 0; b[3] = 0; this.send(b);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const cli = new IrmcClient(opts, {
      onStatus: (s) => {
        ...
        if (s.startsWith('multiuser')) { if (!collected.mode) collected.stage = s; }
