...
1. **Паддинги кредов**: username pad→**16**, password pad→**20**,
   passwordFull pad→**128** (iRMC S2 в нашем server/sdk/avr/irmc.js: 48/48/228;
   параметризовано через `opts.pad`).
...
- Реализовано: **собственный** движок HP лежит внутри модуля —
  `server/platforms/hp-lo100/avr/irmc.js` (+ `avr/irmc-decode.js`), отличие
  задаётся `opts.pad` (`{user:16, pass:20, full:128}`) в
  `platforms/hp-lo100/index.js`. Ядро и движок `mahogany-avr` НЕ изменялись —
  HP-специфику (парити паддингов, view-only, токен) не навязываем S2.
- Автодетект LO100i vs iRMC по handshake не различить (одинаковы) — модуль
  выбирается пробой веб-страницы (`platforms/hp-lo100/probe.js`), а не по
  рукопожатию.
