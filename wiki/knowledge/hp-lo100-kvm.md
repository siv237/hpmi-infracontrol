...
| Поле | HP LO100 | iRMC S2 (наш server/sdk/avr/irmc.js) |
|---|---|---|
...
## Модуль платформы: `platforms/hp-lo100/` — РЕАЛИЗОВАН

Самодостаточная папка модуля (ядро не тронуто). Собственный AVR-движок лежит
**внутри модуля**: `platforms/hp-lo100/avr/irmc.js` + `avr/irmc-decode.js`
(копия движка iRMC-семейства с HP-поведением; общий `sdk/` не изменялся,
`mahogany-avr` пользуется своим прежним движком без правок).

- `login.js` — `fetchKvmApplet(cfg)` (digest GET `/kvms.html` с ретраями и
  паузами ≥2 c → парс APPLET-параметров) + `login()` → sessionCfg;
- `index.js` → `createConsole` — `IrmcClient` из `./avr/irmc.js` с
  `pad: {user:16, pass:20, full:128}`, `port=NonSecure_KVMPort`,
  `httpdata=токен из kvms.html`;
- `probe.js` — матч по title `BMC HTTP Server` / `kvms.html` (APPLET+httpdata),
  локальный `plainGet`;
- `test.js` — самотест (матч-функции + профиль паддингов).
...
(PNG ~11.5 КБ). Важный quirk HP: LO100 стартует сессию **view-only** —
движок модуля разбирает `MultiUserState` и повторно просит полный контроль,
иначе BMC молча дропает нажатия клавиш (HP-специфика, поэтому и держим
отдельный движок — нельзя навязывать это общему S2-движку).
