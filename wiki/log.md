...
## [2026-09-11] fix | регрессия цветов S2 от правки vnc.js (общий мост)
Владелец: правка цветов для S4 сломала цвета на S2. Стратегический вывод:
общий RFB-мост vnc.js не должен компенсировать чей-либо формат пикселей.
Разбор:
- Инвариант проекта: fb.pix = 0x00RRGGBB (R — старший байт). Заявлен в
  irmc-decode.js (S2, IrmcFramebuffer) и console-ivtp.js (S4, IvtpFramebuffer).
- S4 console-ivtp drle_PIII: pB=0,pG=n8,pR=2*n8 -> (r<<16)|(g<<8)|b = RGB (ок).
- S2 irmc-decode drle (стр.337) собирал red|green<<8|blue<<16 = 0x00BBGGRR —
  ЕДИНСТВЕННЫЙ путь из четырёх, нарушавший инвариант (остальные: raw bitBlt,
  2-4-2 RLE, 1bpp planar — уже RGB). Старый vnc.js выдавал B,G,R и тем самым
  «компенсировал» BGR S2; после перехода vnc.js на канон R/B на S2 поменялись.
Фикс: irmc-decode.js drle приведён к канону 0x00RRGGBB (blue|green<<8|red<<16).
Общий vnc.js — только конверсия канона в формат клиента (добавлен явный
контракт-комментарий: формат пикселей — знание движка, не моста).
Принцип: S2 и S4 — раздельные движки (irmc-decode.js / console-ivtp.js),
у каждого свой формат; общий мост работает исключительно с каноном.
Проверено: синтаксис; живой снапшот S2 (nano) не затронут (монохром).
Нужна проверка цветного экрана S2 после перезапуска сервера.

## [2026-09-11] ingest | каркас модулей платформ (плагинная архитектура)
По решению владельца (проект в открытый доступ: любой человек/ИИ добавляет
поддержку новой платформы одной папкой):
- server/platforms/README.md — манифест разработчика (правила, структура,
  шаблон, чеклист).
- server/platforms/_template/ — скелетон модуля (manifest.js, index.js,
  probe.js, README.md): копируй и заполняй.
- server/platforms/mahogany-avr/ , ami-soc/ — заготовки под перенос наших
  S2 и S4 (manifest с supported[], access, capabilities, probes; README с
  картой переноса и граблями). Код переносим по живому железу.
- server/sdk/contracts.js — контракты (PlatformModule, ConsoleClient,
  MediaRedirector, Sdk), версия SDK.
- server/sdk/registry.js — сканирует platforms/, читает manifest, сортирует
  по priority, изолирует ошибки; CLI: node server/sdk/registry.js.
Ключевые решения: пробы/порты/доступ — в папке модуля; креды — универсальны
(ядро), а схема входа — платформенный код; IPMI платформо-зависим, точное
перечисление возможностей; один файл-список supported (модель+прошивка);
ошибка модуля не роняет ядро; канон пикселей 0x00RRGGBB.
